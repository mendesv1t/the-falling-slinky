    ...
    for i in range(n):
        ...
        if i != 0:
            if positions[i - 1] <= initial_positions[i]:
                ...
                if i == n-1:
                    velocities[i] = velocities[i-1] - 2.5
            else:
                accelerations[i] = 0
...
first_point, = ax.plot([], [], 'o', color='red', markersize=0, label='Primeira Massa', zorder=3)
...
def spring_curve(x0, y0, x1, y1, num_coils=3, amplitude=raio_bolinha):
    y0_corrigido = y0 + raio_bolinha
    y1_corrigido = y1 - raio_bolinha
...
def init():
    ...
    return first_point, spring_lines
...
plt.title('Simulação Visual da Queda do Slinky - "Realista"', x=-0.4)
plt.legend()
plt.show()
